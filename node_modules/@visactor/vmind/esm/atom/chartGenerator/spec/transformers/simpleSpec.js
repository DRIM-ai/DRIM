...
const handleMaybeArray = (spec, handler) => isArray(spec) ? spec.map(handler) : handler(spec), combineTitle = titleSpec => {
    ...
    return Object.values(titlesByOrient).forEach((titles => {
        2 === titles.length && titles.every((title => isNil(title.text))) ? res.push(Object.assign(Object.assign({}, titles[0]), {
            text: titles[0].text,
...
export const fomartSimpleSpec = context => {
    ...
    if (simpleVChartSpec) {
        ...
        simpleVChartSpec.title ? spec.title = isArray(simpleVChartSpec.title) && simpleVChartSpec.title.length >= 2 ? combineTitle(simpleVChartSpec.title) : simpleVChartSpec.title : "title" in spec && delete spec.title, 
        simpleVChartSpec.axes ? spec.axes ? (array(simpleVChartSpec.axes).forEach((axis => {
        ...
        })), spec.axes = spec.axes.filter((specAxis => simpleVChartSpec.axes.some((axis => axis.type === specAxis.type && axis.orient === specAxis.orient))))) : spec.axes = array(simpleVChartSpec.axes).map((axis => {
